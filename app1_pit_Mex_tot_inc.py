...
reform = Calculator.read_json_param_objects('app0_reform_pit_Mex_A5gen_allowance.json', None)
...
dumpdf = dumpdf[dumpdf['tot_inc'] > 0]
dumpdf = dumpdf.sort_values(by='tot_inc')

# Extract arrays
values_inc = dumpdf['tot_inc'].dropna().values
...
df_filtered = dumpdf[dumpdf['tot_inc'] > 0].copy()

# Compute ETRs
df_filtered['ETR'] = df_filtered['pitax1'] / df_filtered['tot_inc']
df_filtered['ETR_ref'] = df_filtered['pitax2'] / df_filtered['tot_inc']
...
df_filtered['percentile'] = pd.qcut(df_filtered['tot_inc'], 100, labels=False) + 1
...
min_wage_percentile = (
    df_filtered[df_filtered['tot_inc'] <= min_wage_annual]['weight'].sum() /
    df_filtered['weight'].sum()
) * 100

# Percentile for 1.2x minimum wage
s_min_wage_percentile = (
    df_filtered[df_filtered['tot_inc'] <= s_min_wage_annual]['weight'].sum() /
    df_filtered['weight'].sum()
) * 100

# Percentile for GDP per capita
gdp_pc_percentile = (
    df_filtered[df_filtered['tot_inc'] <= GDP_pc]['weight'].sum() /
    df_filtered['weight'].sum()
) * 100
...
plt.plot(etr_summary['percentile'], etr_summary['ETR'], label="ETR Current Law", linewidth=2, color='gold')
...
plt.xlabel("Income Percentile")
plt.ylabel("Effective Tax Rate")
plt.title("Effective Tax Rate (ETR) by Percentile for Total Gross Income including cash transfers")
plt.xticks(np.arange(0, 101, 10))
plt.ylim(0, 0.25)
plt.grid(alpha=0.3)
plt.legend()
...
#under this approach the plot shows that each curve has a slighltly different distribution, that is why the total_gross_income observation seems lagged
# 1) Compute the two ETR series
dumpdf['ETR_tot']   = dumpdf['pitax1'] / dumpdf['tot_inc']
dumpdf['ETR_gross'] = dumpdf['pitax1'] / dumpdf['total_gross_income']

# 2) Clean out infinities and missing
dumpdf = dumpdf.replace([np.inf, -np.inf], np.nan)
dumpdf = dumpdf.dropna(subset=['ETR_tot', 'ETR_gross', 'tot_inc', 'total_gross_income', 'weight'])
dumpdf = dumpdf[(dumpdf['tot_inc'] > 0) & (dumpdf['total_gross_income'] > 0)]

# 3) Create separate percentile ranks
dumpdf['pct_tot_inc'] = pd.qcut(dumpdf['tot_inc'], 100, labels=False) + 1
dumpdf['pct_gross']   = pd.qcut(dumpdf['total_gross_income'], 100, labels=False) + 1

# 4) Aggregate weighted-average ETR by each ranking
summary_tot = (
    dumpdf
    .groupby('pct_tot_inc')
    .apply(lambda x: np.average(x['ETR_tot'],   weights=x['weight']))
    .reset_index(name='ETR_tot')
)

summary_gross = (
    dumpdf
    .groupby('pct_gross')
    .apply(lambda x: np.average(x['ETR_gross'], weights=x['weight']))
    .reset_index(name='ETR_gross')
)

# 5) Plot both on the same axes, using their own percentiles
plt.figure(figsize=(10, 6))
plt.plot(
    summary_tot['pct_tot_inc'],
    summary_tot['ETR_tot'],
    color='orange',
    linewidth=2,
    label='ETR (Total Gross Income including cash transfers)'
)
plt.plot(
    summary_gross['pct_gross'],
    summary_gross['ETR_gross'],
    color='black',
    linewidth=2,
    label='ETR (Gross Income excluding cash transfers)'
)

plt.xlabel('Income Percentile (by concept)')
plt.ylabel('Effective Tax Rate')
plt.title('Current-Law ETR by Percentile: Including vs. Excluding Exempt Income')
plt.xticks(np.arange(0, 101, 10))
plt.ylim(0, 0.25)
plt.grid(alpha=0.3)
plt.legend()
...
#This approach plot them in the same percentile as for the total_gross_income
# --- Compute two current-law ETRs in the same DataFrame ---
dumpdf['ETR_tot']   = dumpdf['pitax1'] / dumpdf['tot_inc']
dumpdf['ETR_gross'] = dumpdf['pitax1'] / dumpdf['total_gross_income']

# Remove infinities / invalids and keep only positive incomes
dumpdf = dumpdf.replace([np.inf, -np.inf], np.nan)
dumpdf = dumpdf[(dumpdf['tot_inc'] > 0) & (dumpdf['total_gross_income'] > 0)]

# Assign percentiles (using total_gross_income for ranking)
dumpdf['percentile'] = pd.qcut(
    dumpdf['total_gross_income'],
    100,
    labels=False
) + 1

# --- Aggregate to get weighted-average ETRs by percentile ---
etr_summary = dumpdf.groupby('percentile').apply(
    lambda x: pd.Series({
        'ETR_tot':   np.average(x['ETR_tot'],   weights=x['weight']),
        'ETR_gross': np.average(x['ETR_gross'], weights=x['weight'])
    })
).reset_index()

# Optional: compute your reference percentiles (min wage, 1.2×, GDPpc)
min_wage_pct = (dumpdf[dumpdf['total_gross_income'] <= min_wage_annual]['weight'].sum()
                / dumpdf['weight'].sum()) * 100
smin_wage_pct = (dumpdf[dumpdf['total_gross_income'] <= s_min_wage_annual]['weight'].sum()
                 / dumpdf['weight'].sum()) * 100
gdp_pc_pct = (dumpdf[dumpdf['total_gross_income'] <= GDP_pc]['weight'].sum()
              / dumpdf['weight'].sum()) * 100

# --- Single plot with both curves ---
plt.figure(figsize=(10, 6))
plt.plot(
    etr_summary['percentile'],
    etr_summary['ETR_tot'],
    label='ETR (Total Gross Income including cash transfers)',
    linewidth=2,
    color='orange'
...
# Reference lines
plt.axvline(x=min_wage_pct, color='gray', linestyle='--', linewidth=1.5)
plt.text(min_wage_pct + 1, 0.02, 'Min Wage', rotation=90, fontsize=9)
plt.axvline(x=gdp_pc_pct, color='gray', linestyle='-.', linewidth=1.5)
plt.text(gdp_pc_pct + 1, 0.02, 'GDP per Capita', rotation=90, fontsize=9)
plt.axvline(x=smin_wage_pct, color='red', linestyle=':', linewidth=1.5)
plt.text(smin_wage_pct + 1, 0.02, '1.2× Min Wage', rotation=90, fontsize=9)
